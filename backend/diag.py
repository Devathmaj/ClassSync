...
print('Timetable: {} (ID: {})'.format(tt.name, tt.id))
...
if schedule:
    ...
    total_slots = len(schedule.working_days) * len(non_break)
    print('Bell Schedule: {} working days, {} periods/day = {} slots/week'.format(
        len(schedule.working_days), len(non_break), total_slots))
    print('Working days: {}'.format(schedule.working_days))
...
for c in classrooms:
    ct = next((f for f in faculties if f.id == c.class_teacher_id), None)
    ct_name = ct.full_name if ct else 'NONE'
    print('  {} (ID: {}) - Class Teacher: {} (ID: {})'.format(c.name, c.id, ct_name, c.class_teacher_id))

print()
print('FACULTIES ({}):'.format(len(faculties)))
for f in faculties:
    print('  {} (ID: {})'.format(f.full_name, f.id))
